...
@section('content')
    <div class="container py-4">
        ...
        <form method="POST" action="{{ route('transport-jobs.update', $job) }}">
            @csrf
            @method('PUT')
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="customer_id" class="form-select" required>
                            ...
                            @foreach ($customers as $c)
                                <option value="{{ $c->id_customer }}" @selected($job->customer_id == $c->id_customer)>
                                    {{ $c->name_customer }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="card-body row g-3">
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="truck_id" class="form-select" required>
                            ...
                            @foreach ($trucks as $t)
                                <option value="{{ $t->id_truck }}" @selected($job->truck_id == $t->id_truck)>
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="driver_id" class="form-select" required>
                            ...
                            @foreach ($drivers as $d)
                                <option value="{{ $d->id_driver }}" @selected($job->driver_id == $d->id_driver)>
                                    {{ $d->name_driver }}
                                </option>
                            @endforeach
                        </select>
