...
@section('namepage')
    <div class="container">
        <h3>แก้ไขใบสั่งซื้อ{{ $invoice->id_iv }}</h3>
    </div>
@endsection
            ...
            <div class="mb-3">
                ...
                <select name="customer_id" class="form-select" required>
                    ...
                    @foreach ($customers as $c)
                        <option value="{{ $c->id_customer }}" @selected($invoice->Customers_id_customer == $c->id_customer)>
                            {{ $c->name_customer }}
                        </option>
                    @endforeach
                </select>
                            ...
                            <tr>
                                    ...
                                    <select name="products[{{ $i }}][product_id]"
                                        ...
                                        @foreach ($products as $p)
                                            <option value="{{ $p->id_product }}" data-price="{{ $p->unit_price }}"
                                                @selected($p->id_product == $d->Products_id_product)>
                                                {{ $p->name_product }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <td>
                                    <input type="number" name="products[{{ $i }}][quantity]"
                                        class="form-control quantity" value="{{ $d->quantity }}">
                                </td>
                                <td>
                                    <input type="number" name="products[{{ $i }}][total_price]"
                                        class="form-control unit-price" value="{{ $d->total_price }}">
                                </td>
                                <td>
                                    <input type="number" class="form-control total-amount" value="{{ $d->total_amount }}">
                                </td>
                                <td>
